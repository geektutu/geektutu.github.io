/**
 * 将 Astro 生成的 post 动态路由目录 /post/<slug>.html/ 转换为真正的
 * /post/<slug>.html 静态文件，兼容原博客 URL，且让 Pagefind 能正确索引。
 */
...
const POST_DIR = join(ROOT, 'dist/post');

if (!existsSync(POST_DIR)) {
  console.log('未找到 dist/post，跳过 .html 转换。');
  process.exit(0);
}
...
console.log(`✔ 已将 ${converted} 篇文章目录转换为 /post/*.html 文件。`);
